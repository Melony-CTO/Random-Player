    ...
    setupAudioEventListeners() {
        ...
        this._audioEventHandlers = {
            ...
            canplay: () => {
                ...
                // ✅ 재생 가능 시점에도 버튼 상태 업데이트
                this.uiManager.updatePlayButton(this.audioManager.isPlaying);
                this.updateLandscapePlayButton(this.audioManager.isPlaying);
            },
...
error: (e) => {
    ...
    async playNextTrack() {
        ...
        // ✅ 안전장치: 10초 후 강제로 플래그 해제 (무한 대기 방지)
        const safetyTimeout = setTimeout(() => {
            if (this.isTrackSwitching) {
                console.warn('⚠️ 트랙 전환 타임아웃 (10초) - 강제 플래그 해제');
                this.isTrackSwitching = false;
            }
        }, 10000);
...
const audioFiles = Array.from(files).filter(file => {
    ...
    /**
     * ✅ 가로모드 플레이 버튼 업데이트 (새로 추가)
     * @param {boolean} isPlaying - 재생 중 여부
     */
    updateLandscapePlayButton(isPlaying) {
        const landscapeBtn = document.getElementById('landscapePlayButton');
        if (landscapeBtn) {
            landscapeBtn.innerHTML = isPlaying ? '⏸' : '▶';
            console.log('🎵 가로모드 플레이 버튼 업데이트:', isPlaying ? '⏸' : '▶');
        }
    }
}
